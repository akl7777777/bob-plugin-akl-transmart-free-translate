...
function initData(source_lang, target_lang, translate_text) {
    return {
        "header": {
            "fn": "auto_translation",
...
function translate(query, completion) {
    (async () => {
        ...
        if (translate_text !== '') {
            const url = 'https://yi.qq.com/api/imt';
            const post_data = initData(source_lang, target_lang, translate_text);
            ...
            try {
                $http.request({
                    ...
                    handler: function (resp) {
                        if (resp.data && resp.data.auto_translation && resp.data.auto_translation.length) {
                            completion({
                                result: {
                                    from: query.detectFrom,
                                    to: query.detectTo,
                                    toParagraphs: resp.data.auto_translation[0].split('\n'),
                                },
                            });
                        } else {
